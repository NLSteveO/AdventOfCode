...
const getLineCoordinates = (start, end) => {
  ...
  if (x1 === x2) {
    ...
    coordinates.push('xy');
    const xShouldAdd = x2 > x1;
    const yShouldAdd = y2 > y1;
    const length = Math.abs(x1 - x2);
    for (let i = 0; i <= length; i++) {
      const x = xShouldAdd ? x1 + i : x1 - i;
      const y = yShouldAdd ? y1 + i : y1 - i;
...
const plotLine = (chart, lineCoordinates) => {
  ...
  let high;
  if (axis !== 'xy') {
    axisValue = lineCoordinates.shift();
    low = lineCoordinates.shift();
    high = lineCoordinates.shift();
  }
  if (axis === 'x') {
    ...
    for (let i = 0; i < lineCoordinates.length; i++) {
      const [x, y] = lineCoordinates[i];
      const value = chart[y][x];
      let newValue;
      if (value === '.') newValue = 1;
      else newValue = value + 1;
      chart[y][x] = newValue;
